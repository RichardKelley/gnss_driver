...
namespace gnss_driver_ut {
  ...
  static bool stream_comm_test(gnss_driver::Stream *stream) {
    // connect
    int cnt = 0;
    uint8_t buffer[1024];
    size_t data_len = 128;
  ...
  TEST(Stream, TcpUdp) {
    ...
    gnss_driver::Stream *tcp_stream = gnss_driver::Stream::create_tcp("127.0.0.1", TCP_PORT, 1000);
    ...
    gnss_driver::Stream *udp_stream =
	gnss_driver::Stream::create_udp("127.0.0.1", UDP_PORT, 1000);
    ...
    s_server_ready = 0;
    gnss_driver::Stream *searial_stream =
      gnss_driver::Stream::create_serial(FLAGS_serial_port.c_str(), gnss_driver::BAUDRATE_9600,
					 1000);
