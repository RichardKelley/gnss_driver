...
namespace gnss_driver {
  ...
  void SerialStream::open(void) {
    ...
    if (fd == -1) {
      ...
      case EMFILE:
      default:
        ROS_ERROR_STREAM_THROTTLE(2, "Open device " << device_name_
...
#error "OS Support seems wrong."
  ...
  void SerialStream::check_remove() {
    ...
    if (nsent < 0) {
      ROS_ERROR_STREAM("Serial stream detect write failed, error: " << strerror(errno));
      switch (errno) {
      case EBADF:
      case EIO:
	status_ = Stream::Status::DISCONNECTED;
	ROS_ERROR_STREAM("Device " << device_name_ << " removed.");
	disconnect();
	break;
      }
    }
  ...
  size_t SerialStream::read(uint8_t* buffer, size_t max_length) {
	...
	switch (errno) {
        ...
        case EBADF:
        case EIO:
          ROS_ERROR_STREAM("Serial stream read data failed, error: " << strerror(errno));
          disconnect();
          if (connect()) {
  ...
  size_t SerialStream::write(const uint8_t* data, size_t length) {
    ...
    while ((length > 0) && (delay_times < 5)) {
      ...
      if (nsent < 0) {
	ROS_ERROR_STREAM("Serial stream write data failed, error: " << strerror(errno));
	switch (errno) {
        case EAGAIN:
        case EINVAL:
          nsent = 0;
